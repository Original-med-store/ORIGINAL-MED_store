        ...
        <div class="item-card slide-up" style="animation-delay: ${animDelay}s; cursor: pointer;" onclick="openQuickView(${item.id})">
            ...
            <div class="card-body">
                ...
                <div class="card-footer">
                    ${priceHtml}
                    <button class="add-btn" title="أضف للسلة" onclick="event.stopPropagation(); addToCart(${item.id}, \`${item.name}\`, ${item.price}, '${images[0] || FALLBACK_IMG}')">
...
function openQuickView(itemId) {
    ...
    if (addToCartBtn) {
        ...
        newAddToCartBtn.onclick = function () {
            addToCart(item.id, item.name, item.price, imgUrl);
        };
    }
...
function proceedToCheckout() {
    ...
    if (cartSidebar && cartSidebar.classList.contains('show-sidebar')) {
        cartSidebar.classList.remove('show-sidebar');
        setTimeout(() => cartSidebar.classList.add('hidden'), 400); // safely close sidebar
    }
    setTimeout(() => {
        if (checkoutModal) {
